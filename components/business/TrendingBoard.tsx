...
export default function TrendingBoard({ businesses }: { businesses: TrendingBusiness[] }) {
    ...
    <div className="card-soft overflow-hidden">
      {businesses.map((b, i) => (
        <Link
          key={b.id}
          href={`/b/${b.slug}`}
          className="group flex items-center gap-3.5 px-4 sm:px-5 py-4 transition-colors hover:bg-[var(--mist)]"
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            <span className="block font-bold truncate transition-colors group-hover:text-[var(--ivouch-blue)]"
              style={{ color: 'var(--ink)' }}>
              {b.name}
            </span>
            {b.categoryName && (
          ...
          <span className="flex items-center gap-2 flex-shrink-0">
            ...
            <span className="inline-flex items-center gap-1 text-sm font-extrabold"
              ...
              <ShieldCheck size={15} /> {b.vouch_count}
            </span>
            <ArrowRight size={15} className="text-gray-300 transition-transform group-hover:translate-x-0.5" />
          </span>
        </Link>
      ))}
    </div>
